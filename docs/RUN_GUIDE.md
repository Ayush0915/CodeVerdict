# CodeSentry — Step-by-Step Running Guide
...
### 1. Open a PowerShell/Terminal window at the project root (`D:\project\codesentry`).
...
```
> [!NOTE]
> If no `.env` file or `GROQ_API_KEY` is present, CodeSentry automatically falls back to **Deterministic Mock Mode** so you can demo the application fully offline.
...
INFO:     Initializing CodeSentry RAG Vector Store...
...
cd D:\project\codesentry\frontend
```
...
### 1. Open a terminal at the project root (`D:\project\codesentry`).
...
This will run all mock files and generate a detailed report markdown file at **[backend/eval/eval_results.md](file:///D:/project/codesentry/backend/eval/eval_results.md)**.
